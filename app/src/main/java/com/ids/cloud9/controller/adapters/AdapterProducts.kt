        ...
        fun bind(item: ProductListItem) {
            ...
            if (item.unitId != null && MyApplication.units != null && MyApplication.units.size > 0) {
                ...
                if (unit != null)
                    ...
                    binding.tvUnit.text = if(!item.product.unit.name.isNullOrEmpty()) item.product.unit.name else ""
                }

            } else binding.tvUnit.text = if(!item.product.unit.name.isNullOrEmpty()) item.product.unit.name else ""
            }catch (ex:Exception){
                binding.tvUnit.text = ""
            }
            ...
            if (rep != null) {
                ...
                if (MyApplication.selectedVisit!!.reasonId == AppConstants.ARRIVED_REASON_ID) binding.ivReport.setTintImage(
                    R.color.black
                )
            }
